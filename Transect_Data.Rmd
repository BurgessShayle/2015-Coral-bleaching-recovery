...
library(phia)
#Mcap
model1<-art(sum~Reef+MonthFactor+Reef:MonthFactor+(1|Depth)+(1|Rep), data=subset(BS_mcap))
...
#posthoc 
	#testInteractions(artlm(model1, "Reef:MonthFactor"), pairwise=c("Reef","MonthFactor"), model = adjustment="holm")  <- this isn't working great
		
	#OR...for one way model (this is working better I think)
		model.lm<-artlm(model1, "MonthFactor")  #post hoc, get model ready to be passed into emmeans
		marginal<-emmeans(model.lm, ~MonthFactor)
		pairs(marginal, adjust="tukey")
		cld(marginal, alpha=0.05, Letters=letters, adjust="tukey") # this is what we want I think, but only compares Mcap by month, not reef (which is not significant, however interaction is)
		
		#post-hoc for 2-way model
		model.int=artlm(model1, "Reef:MonthFactor")
		marginal=emmeans(model.int, ~Reef:MonthFactor)
    contrast(marginal, method="pairwise", adjust="none")	 #<- some significnt	
		    contrast(marginal, method="pairwise", adjust="tukey")		#none sig, diff between NONE vs TUKEY?
		    
		# post hoc for other interactions: this not as useful!
		    model.diff<-artlm(model1, "Reef:MonthFactor")
		    marginal=emmeans(model.diff, ~Reef:MonthFactor)
        contrast(marginal, method = "pairwise", interaction=T)
