...
Color score for *M. capitata* and *P. compressa* colonies at the Inner Lagoon and Outer Lagoon after the 2015 coral bleaching event in Kaneohe Bay.  Tagged individuals were given a color score based on their visual color: 1) white (>80% of colony white with no visible pigmentation); 2) pale (>10% colony affected by pigment loss); or 3) fully pigmented (<10% colony with any pale coloration). 
...
anova(Mcap_df_mod, type=2) #pvals
 ...
 #posthoc
  emm<-emmeans(Mcap_df_mod,  ~ Site*month)
  pairs(emm)
  
  #4,Month0 - 13,Month0      -0.049784 0.0806 346 -0.618  1.0000 
  #4,Month1.5 - 13,Month1.5  -0.157858 0.0858 377 -1.841  0.8999 
  #4,Month3 - 13,Month3       0.082619 0.0872 383  0.948  0.9999
  #4,Month4.5 - 13,Month4.5   0.119074 0.0854 373  1.394  0.9912 
  #4,Month6 - 13,Month6       0.206844 0.0931 412  2.221  0.6815 
  #4,Month10 - 13,Month10     0.214367 0.0968 427  2.215  0.6859 
  # 4,Month18 - 13,Month18     0.104741 0.1202 478  0.871  1.0000 
  #4,Month24 - 13,Month24     0.156023 0.1256 482  1.242  0.9974
...
(subset of full dataset with only colonies when both partners of a pair are found at each time point)   
Setup df:
```{r,results="hide"}
...
```
# DIFF DATA
Differences (nonbleached-bleached) for pairs over time: df
```{r}
...
```
Mean color scores across the entire project
```{r, results=TRUE}
#calc average bleaching score per grouping by time point
...
anova(dMpigPairall_mod1, type=2)
...
anova(dPpigPairall_mod1, type=2)
