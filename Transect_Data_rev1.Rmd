...
This script is the analysis of the 2015 coral bleaching recovery paper's transect data at the Inner and Outer Lagoons in Kaneohe Bay, Oahu, HI.
...
```{r,results="hide", message=FALSE}
#Setup workspace:
rData1<- read.csv("transect_data_20191022.csv", header=T) # load in transect data
...
Prevalence, peak bleaching, all timepoints and by month: W+P  
Full model:lmer(sum~Reef*MonthFactor*Species+(1|Depth)+(1|Rep)) and plot
```{r}
#fullmodel, df=rData3
    Prev_transect_full<-lmer(sum~Reef*MonthFactor*Species+(1|Depth)+(1|Rep), data=rData3)
anova(Prev_transect_full, type=2)
    #conduct Tukey post hoc test 
    emm = emmeans(Prev_transect_full, ~ Reef*MonthFactor, adjust="tukey")
    #letter display
      pairs(emm) #Inner,0 - Outer,0  p  <.0001
    emm = emmeans(Prev_transect_full, ~ Species*MonthFactor, adjust="tukey")
    #letter display
    pairs(emm) 
    #Mcap,1.5 - Pcomp,1.5    p=  0.0155
    #Mcap,4.5 - Pcomp,4.5    p=  0.7068
    #Mcap,10 - Pcomp,10      p=  0.0001
    #Mcap,1819 - Pcomp,1819  p=  0.1864
       
#plot
# calc average  bleaching rate/month in %  per grouping
transectPlotmeanprev <- ddply(prev_peak, c("Species", "Reef"), summarise, 
                   N    = length(sum[!is.na(sum)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                   mean = mean(sum, na.rm=TRUE), #calculate mean of response variable, removing NA's
                   sd   = sd(sum, na.rm=TRUE), #calculate standard deviation
                   se   = sd / sqrt(N), #calculate standard error
                   max = max(sum, na.rm=TRUE) #calculate max, could also calculate min () if desired
)

transectPlotmeanprev$code<-paste0(transectPlotmeanprev$Species, transectPlotmeanprev$Reef)

prev_plot<-ggplot(data=transectPlotmeanprev, aes(x=Reef, y=mean, group = Species, color=Species)) + 
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1)+
  geom_point(aes(shape=code), size=3)+
  #geom_line(aes(color=code, linetype=code))+
  #scale_linetype_manual(values=c("solid", "dotted"))+
  #scale_color_manual(values=c("black", "black","black"))+
  ylab("Percent") +
    xlab("Lagoon") + #Label the X Axis
    #ylim(1, 3) + #set Y limits
  theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        axis.title=element_text(size=14,face="bold"), #Set axis format
        ...
        plot.background =element_blank(), #Set the plot background
        legend.key = element_blank()) + #Set plot legend key
  ggtitle("Peak bleaching, prev")+
          theme(plot.title = element_text(size=20,face = "italic",hjust = 0.5));prev_plot
```
...
anova(WhiteMod, type=2)
  ...
  #conduct Tukey post hoc test 
    ...
    cld(emm)
    pairs(emm)
   # Inner,Mcap,0 - Inner,Pcomp,0        -44.84422 4.3 58 -10.420 <.0001
    #Outer,Mcap,0 - Outer,Pcomp,0          1.02260 4.3 58   0.238 1.0000
```

##Severity (white/white+pale)
Supplemental table 6 (setup)
```{r}
...
anova(SeverityMod, type="2")
 ...
 #conduct Tukey post hoc test 
    ...
    cld(emm)
    pairs(emm) 
    #Mcap,0,Inner - Pcomp,0,Inner        -0.428518 0.0789 41.0 -5.432  0.0004 
    #Mcap,0,Outer - Pcomp,0,Outer         0.044986 0.0789 41.0  0.570  1.0000
 #conduct Tukey post hoc test 
    ...
    #Mcap,0,Outer - Pcomp,0,Outer         0.044986 0.0789 41.0  0.570  1.0000
    #Mcap,0,Inner - Pcomp,0,Inner        -0.428518 0.0789 41.0 -5.432  0.0004
    #Pcomp,0,Inner - Pcomp,0,Outer        0.515064 0.0789 41.0  6.529  <.0001 
    #Mcap,0,Inner - Mcap,0,Outer          0.041560 0.0789 41.0  0.527  1.0000
```
