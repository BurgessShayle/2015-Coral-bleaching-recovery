...
This script is for the 2015 bleaching recovery paper's transect data at the Inner and Outer Lagoons. Here, we look at std error in the transect replicates. 

*This script includes a look at depth, it's not significant at any timepoint but 4.5 months. We are removing it from the analyses.*   
...
Prevalence all timepoints and by month - W+P
Mcap by Lagoon at each month
```{r}
#FULL MODEL 

Mcap_transect_mod<-lmer(sum~Reef*MonthFactor+(1|Depth)+(1|Rep), data=BS_mcap)
anova(Mcap_transect_mod)

#conduct Tukey post hoc test 
    emm = emmeans(Mcap_transect_mod, ~ MonthFactor, adjust="tukey")
    #letter display
    cld(emm)
   
    
#full mod with species added - do we want this?
    Mcap_transect_mod_species<-lmer(sum~Reef*MonthFactor*Species+(1|Depth)+(1|Rep), data=rData3)
anova(Mcap_transect_mod_species)
    #conduct Tukey post hoc test 
    emm = emmeans(Mcap_transect_mod_species, ~ MonthFactor:Species, adjust="tukey")
    #letter display
    cld(emm)
         pairs(emm)
...
#mcap 0
BSc_mcap0<-lmer(sum~Reef+(1|Depth)+(1|Rep), data=subset(BS_mcap, MonthFactor=="0"))
anova(BSc_mcap0)
#conduct Tukey post hoc test 
    emm = emmeans(BSc_mcap0, ~ Reef, adjust="tukey")
    #letter display
    cld(emm)
   

#mcap 1.5
BSc_mcap1.5<-lmer(sum~Reef+(1|Depth)+(1|Rep), data=subset(BS_mcap, MonthFactor=="1.5"))
anova(BSc_mcap1.5)

#mcap 4.5
BSc_mcap4.5<-lmer(sum~Reef+(1|Depth)+(1|Rep), data=subset(BS_mcap, MonthFactor=="4.5"))
anova(BSc_mcap4.5)

#mcap 10
BSc_mcap10<-lmer(sum~Reef+(1|Depth)+(1|Rep), data=subset(BS_mcap, MonthFactor=="10"))
anova(BSc_mcap10)

#mcap 1819
BSc_mcap1819<-lmer(sum~Reef+(1|Depth)+(1|Rep), data=subset(BS_mcap, MonthFactor=="1819"))
anova(BSc_mcap1819)
...
pcomp_transect_mod<-lmer(sum~Reef*MonthFactor+(1|Depth)+(1|Rep), data=BS_pcomp)
anova(pcomp_transect_mod)

#conduct Tukey post hoc test 
    emm = emmeans(pcomp_transect_mod, ~ MonthFactor, adjust="tukey")
    #letter display
    cld(emm)
    #conduct Tukey post hoc test 
    emm = emmeans(pcomp_transect_mod, ~Reef, adjust="tukey")
    #letter display
    cld(emm)
   #conduct Tukey post hoc test 
    emm = emmeans(pcomp_transect_mod, ~Reef*MonthFactor, adjust="tukey")
    #letter display
    cld(emm)
    
#pcomp 0
BSc_pcomp0<-lmer(sum~Reef+(1|Depth)+(1|Rep), data=subset(BS_pcomp, MonthFactor=="0"))
anova(BSc_pcomp0)
#conduct Tukey post hoc test 
    emm = emmeans(BSc_pcomp0, ~ Reef, adjust="tukey")
    #letter display
    cld(emm)
   

#pcomp 1.5
BSc_pcomp1.5<-lmer(sum~Reef+(1|Depth)+(1|Rep), data=subset(BS_pcomp, MonthFactor=="1.5"))
anova(BSc_pcomp1.5)
#conduct Tukey post hoc test 
    emm = emmeans(BSc_pcomp1.5, ~ Reef, adjust="tukey")
    #letter display
    cld(emm)

#pcomp 4.5
BSc_pcomp4.5<-lmer(sum~Reef+(1|Depth)+(1|Rep), data=subset(BS_pcomp, MonthFactor=="4.5"))
anova(BSc_pcomp4.5)

#pcomp 10
BSc_pcomp10<-lmer(sum~Reef+(1|Depth)+(1|Rep), data=subset(BS_pcomp, MonthFactor=="10"))
anova(BSc_pcomp10)

#pcomp 1819
BSc_pcomp1819<-lmer(sum~Reef+(1|Depth)+(1|Rep), data=subset(BS_pcomp, MonthFactor=="1819"))
anova(BSc_pcomp1819)
...
```{r} 
ccData<-read.csv("benthicTransectCoralcover20191024.csv", header=T)
ccData$MonthFactor<-ccData$Month
ccData$MonthFactor[ccData$MonthFactor=="18"] <-"1819"
...
Coral cover full model lmer (not in paper yet)
```{r}

CoralCoverMod<-lmer(CoralCover~Reef*MonthFactor+(1|Depth)+(1|Survey), ccData)
anova(CoralCoverMod)
...
ccMonth0<-lmer(CoralCover~Reef+(1|Depth)+(1|Survey), ccMonth0)
anova(ccMonth0) 

```

SI Table 4: CoralCover~Month at each Lagoon  <- delete
```{r} 
#SI2
...
WhiteMod<-lmer(Percent~Species*Reef*MonthFactor+(1|Depth)+(1|Rep),prevWhite)
anova(WhiteMod)
#all significant 
#conduct Tukey post hoc test 
...
M0PrevWhite<-subset(prevWhite, MonthFactor=="0")
M0PrevWhiteMod<-lmer(Percent~Reef*Species+(1|Depth)+(1|Rep),M0PrevWhite)
anova(M0PrevWhiteMod) 
 #conduct Tukey post hoc test 
    emm = emmeans(M0PrevWhiteMod, ~ Species, adjust="tukey")
    #letter display
    cld(emm)
 #conduct Tukey post hoc test 
    emm = emmeans(M0PrevWhiteMod, ~ Reef*Species, adjust="tukey")
    #letter display
    cld(emm)   
    
    
    
# we shouldn't do by reef    
...
# get rid of Pale
rData4<-subset(rData4, Bleaching_status=="Bleached")
...
```{r}
#full model
SeverityMod<-lmer(newPer~Species*Reef*MonthFactor+(1|Depth)+(1|Rep),rData4)
anova(SeverityMod)
 ...
 #conduct Tukey post hoc test 
    ...
    pairs(emm)
 #conduct Tukey post hoc test 
    emm = emmeans(SeverityMod, ~ Reef*Species, adjust="tukey")
    #letter display
    cld(emm)   



#previously how you did it.
